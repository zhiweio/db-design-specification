...
MySQL InnoDB 及兼容 MySQL 的数据库参考改规范。
...
* 表名以 `ods_`、`dwd_`、`dws_`、`dim_`、`ads_` 等前缀开头。

* 临时表名称必须以 `tmp_` 为前缀，并以日期为后缀。
...
* 物理主键：命名为 `id`，类型为 `INT` 或 `BIGINT`，且为 `auto_increment`，且主键值禁止被更新

* 业务主键：用业务字段约定表的唯一性，确定其增删改逻辑，建表之初必须确认业务主键，建立唯一索引。

* 禁止使用外键。

* 关联表的父表应有主健，主健字段或组合字段应满足非空属性和唯一性要求。
...
* 外部时间：由数据导入或外部应用程序产生的日期时间类型采用 `VARCHAR` 类型 。
...
1. 数字编码化，如 patent_id、person_id、币种、城市码。
...
1. 数据的公布日期和发生日期均要采集。
2. 重要数据加快轮巡频率。
3. 留存数据创建日期（create_time）和数据更新日期（update_time）。



### 预定义字段

每张表必须包含固定字段，如需使用，按标准添加常用字段。
...
| ----------- | ------------ | -------- | ------ | -------------- |
| id          | 自增主键     | BIGINT   |        |                |
| is_history  | 是否历史     | TINYINT  | 0      | 0：否，1：历史 |
| create_time | 创建时间     | DATETIME |        |                |
| update_time | 记录更新时间 | DATETIME |        |                |
...
| personal_name | 个人姓名       | VARCHAR(128)  |        |                            |
...
* 【建议】数量限制：索引数量控制在 7 个以内，最多不可以超过表字段个数的 20%。
...
1. 【强制】开发人员禁止拥有任何环境 `DLL` 权限账号
2. 【强制】版本发布需在指定目录提交线上`SQL`变更需求，必须详细注明提交人、日期、需求说明
3. 【强制】大数据量的数据操作存在锁表等风险的需要提出风险预警，避开业务高峰期执行
4. 【强制】禁止直接在线上库执行后台管理和统计类查询
