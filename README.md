...
* 表名以 `ods_`、`dwd_`、`dws_`、`dim_`、`ads_` 等前缀开头（在 MySQL 中请使用 `t_` 作为前缀）。
...
| ----------- | ------ | --------- | ----------------- | --------------------------- |
| id          | 自增主键   | BIGINT    |                   |                             |
| create_time | 创建时间   | TIMESTAMP | CURRENT_TIMESTAMP |                             |
| update_time | 记录更新时间 | TIMESTAMP | CURRENT_TIMESTAMP | ON UPDATE CURRENT_TIMESTAMP |
...
* 【建议】数量限制：索引数量控制在 5 个以内，最多不可以超过表字段个数的 20%（索引有写入成本？）。
